...
// no mesmo servidor — usamos o mesmo domínio em que o site está.
// ===================================================
const API_BASE_URL = window.location.origin.includes("file://") ? "http://localhost:8000" : "";
...
async function preencherFigurinhas() {
    try {
        ...
        for (const slot of slots) {
            ...
            slot.addEventListener("click", () => abrirModalDetalhes(id));
        }
...
// ===================================================
// MODAL DE DETALHES DAS PROVÍNCIAS
// ===================================================
...
async function abrirModalDetalhes(id) {
    try {
        const response = await fetch(`${API_BASE_URL}/figurinhas/${id}`);
        if (!response.ok) throw new Error("Detalhes não encontrados");
        
        const detalhes = await response.json();
        
        // Preencher Modal
        document.getElementById("modal-title").textContent = detalhes.nome;
        document.getElementById("modal-img").src = `${API_BASE_URL}${detalhes.imagem_url}`;
        
        document.getElementById("modal-capital").textContent = detalhes.capital || "Não informada";
        document.getElementById("modal-populacao").textContent = detalhes.populacao || "Desconhecida";
        
        document.getElementById("modal-curiosidade").textContent = detalhes.curiosidade || "Sem informações disponíveis no momento.";
        
        const distritosContainer = document.getElementById("modal-distritos");
        distritosContainer.innerHTML = "";
        
        if (detalhes.distritos && detalhes.distritos.length > 0) {
            detalhes.distritos.forEach(distrito => {
                const chip = document.createElement("span");
                chip.className = "chip";
                chip.textContent = distrito;
                distritosContainer.appendChild(chip);
            });
        } else {
            distritosContainer.innerHTML = "<p>Sem distritos mapeados.</p>";
        }
        
        // Exibir modal
        modal.classList.remove("hidden");
    } catch (erro) {
        console.error("Erro ao buscar detalhes da província:", erro);
        alert("Não foi possível carregar os detalhes desta província.");
    }
}
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (pageFlip) {
        ...
        pageFlip.on("flip", (e) => {
            ...
            // Show 'Back to Index' button only if past the index page (page > 1)
            if (btnBackIndex) {
                if (currentPage > 1) {
                    btnBackIndex.classList.remove("hidden");
                } else {
                    btnBackIndex.classList.add("hidden");
                }
            }
        });
        ...
        if (btnBackIndex) {
            btnBackIndex.addEventListener("click", () => {
                pageFlip.flip(1); // Page 1 is the Index
            });
        }

        // Click events for interactive index / region buttons with data-page attribute
        document.querySelectorAll("[data-page]").forEach((btn) => {
            btn.addEventListener("click", (e) => {
                const targetPage = parseInt(e.currentTarget.getAttribute("data-page"), 10);
                if (!isNaN(targetPage) && pageFlip) {
                    e.currentTarget.classList.add("pulse-highlight");
                    setTimeout(() => {
                        e.currentTarget.classList.remove("pulse-highlight");
                    }, 300);
